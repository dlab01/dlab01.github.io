# AGENTS.md — DLAB MGT 사이트

> 디랩 코딩학원 대치점 MGT반 수업 사이트. GitHub Pages 정적 호스팅.
> 전역 규칙(`~/.Codex/AGENTS.md`)을 따르되, 충돌 시 **이 파일이 우선**한다.
...
5. **요청 안 한 리팩터링·"개선"을 하지 마라.** 손댄 모든 줄이 요청에 직접 연결돼야 한다.
   인접 코드 정리·포맷 변경 금지(전역 AGENTS.md §3 "Surgical Changes" 적용).
...
- 데스크톱 폭(예: 1440px)에서 레이아웃 확인. 모바일 최적화는 기본 범위가 아니다.
